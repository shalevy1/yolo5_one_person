...
def extract_json_data(jsons_path, meta_dir, save_filename, images_dir, key_name='photo', ext='txt', mode='train', zfill_num = 9):
    """Create a txt file with all images names that are present in the given set of jsons and train/test sets"""
    jsons_list = sorted(search_in_dir_for_files(dir_path=jsons_path, search_phrase=f'{mode}_*.json'))

    with open(os.path.join(meta_dir, save_filename), 'w') as f:
        for jsonn_path in jsons_list:
            jsonn = load_json(jsonn_path)
            # print(f'extract json data lenght: {len(jsonn)}')
            for item in jsonn:
                image_name = str(item.get(f'{key_name}')).zfill(zfill_num) + f'.{ext}'
                f.write(os.path.join(images_dir, image_name) + '\n')

def merge_train_test_subsets(filenames, meta_dir, save_filename):
    with open(os.path.join(meta_dir, save_filename), 'w') as f:
        # print(f'Merge train test subset master path: {os.path.join(meta_dir, save_filename)}')
        # print(f'Filenames to save: {filenames}')
        for item in filenames:
            with open(os.path.join(meta_dir, item), 'r') as ff:
                temp = ff.readlines()
                # print(len(temp))
                f.writelines(temp)

def read_yolo_annotations_from_txt(file_abs_path):
    with open(file_abs_path, 'r') as f:
        annots = f.readlines()
    return [item.strip() for item in annots]
...
def get_images_size(path):
    # print(f'get_images_size path: {path}')
...
def create_category_txt_filepaths(categories_dict, meta_dir, images_dir, mode='single'):
    categories_jsons = list(categories_dict.keys())

    if mode == 'all':
        temp_list = []
        temp_list.append(categories_jsons)
        categories = temp_list
    ...
    for category in categories_jsons:
        ...
        product_filenpaths = [os.path.join(images_dir, item) for item in product_filenames]
        # print(product_filenpaths)
        exisitng_product_filenames = check_if_images_exists_save(product_filenpaths, train_file_path=None, save=False)
        product_filenames = [os.path.basename(item) for item in exisitng_product_filenames]
        ...
        with open(os.path.join(meta_dir, f'{category}_products.txt'), 'w') as f:
            for product in product_filenames:
                f.write(product + '\n')

def create_index_of_images(save_dir, product_filenames):
    # n = len(product_filenames)
    temp = []
